...
// V2: Single channel, no batching, shared memory loading changed to improve coalescence and reduce serial loads
...
__global__ void conv2d(int *in, int *out, int *kernel)
{
    ...
    if (gidy >= DSIZE + HALO || gidx >= DSIZE + HALO)
        return;
    ...
    int tile_start = blockIdx.x * blockDim.x + blockIdx.y * blockDim.y * DSIZE;
    for (int lindex = threadIdx.x + blockDim.y * threadIdx.y;
         lindex < (blockDim.x + 2 * HALO) * (blockDim.y + 2 * HALO);
         lindex += blockDim.x * blockDim.y)
    {
        int l_thread_row = lindex / (blockDim.x + 2 * HALO);
        int l_thread_col = lindex - (blockDim.x + 2 * HALO) * l_thread_row;
        lin[lindex] = in[tile_start + l_thread_col + DSIZE * l_thread_row];
    }
    ...
    for (int i = -HALO; i <= HALO; ++i)
    {
        ...
        {
            value += lkernel[i + HALO][j + HALO] * lin[lidx + i + (lidy + j) * (blockDim.x + 2 * HALO)];
        }
    }
